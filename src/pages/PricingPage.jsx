        ...
        <div className="grid gap-6 xl:grid-cols-3">
          {pricing.map((tier, index) => (
            <Reveal key={tier.tier} delay={0.08 * index}>
              <GlassCard
                className={`relative h-full p-8 sm:p-10 ${tier.featured ? 'shine-card border-white/22 bg-white/10 shadow-[0_30px_80px_rgba(100,90,255,0.18)]' : 'shine-card'}`}
              >
                {tier.featured && (
                ...
                <p className="text-xs font-semibold uppercase tracking-[0.34em] text-[#f6efe2] drop-shadow-[0_0_18px_rgba(203,178,139,0.34)]">{tier.tier}</p>
                <p className="mt-5 text-base leading-8 text-white/64">{tier.subtitle}</p>
